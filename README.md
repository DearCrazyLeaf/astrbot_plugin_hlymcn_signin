# HLYM Server Toolkit (AstrBot Plugin)

### 💬 群聊签到 + 🎮 服务器查询（A2S/后端 API）+ 📊 数据查询 + 🛠️ RCON

![Python](https://img.shields.io/badge/Python-3.10%2B-blue)
![AstrBot](https://img.shields.io/badge/AstrBot-v4.12%2B-brightgreen)
![License](https://img.shields.io/badge/License-GPL--3.0-orange)

![views](https://count.getloli.com/get/@astrbotchuanhuatong?theme=booru-jaypee)

---

## 🧩 插件定位

这是一个面向 QQ 群场景的服务器工具插件，支持：
- ✅ 群聊签到
- ✅ 查服（后端 API 或 A2S 直连）
- ✅ 玩家数据查询
- ✅ RCON 远程指令
- ✅ 图片卡片渲染

---

## ⚠️ 重要说明（请先看）

> [!IMPORTANT]
> 本插件高度依赖后端。  
> 本仓库只提供 AstrBot 插件，不提供后端服务实现。

如果你没有自建后端，通常只能使用“基础查服（A2S 直连）”这类浅层功能。  
签到、数据查询、RCON、地图封面背景等核心能力都需要你自己的后端接口支持。

---

## 📌 功能依赖矩阵

| 功能 | 无后端可用 | 需要后端 | 说明 |
|------|------------|----------|------|
| 查服（A2S直连） | 是 | 否 | 配置 `servers` 即可，接口失败可回退 A2S |
| 查服（后端API） | 否 | 是 | 需要 `server_status_api_base_url + server_status_api_path` |
| 查服背景 `header` | 是 | 否 | 使用插件头图缓存 |
| 查服背景 `map_cover` | 否 | 是 | 需要后端返回 `mapImage/map_image/basic_info.map_image` |
| 签到 | 否 | 是 | 需要签到接口 |
| 数据查询 | 否 | 是 | 需要 QQ 数据查询接口 |
| RCON | 否 | 是 | 需要 RCON 后端 API |

---

## 🚀 快速开始

### 🟢 方案 A：仅使用基础查服（无需后端）

最小配置建议：
- `allowed_group_ids`：允许触发功能的群号
- `servers`：服务器别名列表（`别名=ip:port`）
- `a2s_render_mode`：`card` / `image_text` / `text`
- `server_status_api_base_url`：留空（让插件走 A2S 直连）
- `server_query_bg_mode`：建议 `header`
- `header_image_url`：可选，提供查服背景图

可用命令：
- 发送服务器别名（例如 `cs`）触发查服
- 发送 `ip` 查看已配置服务器别名列表

### 🔵 方案 B：使用完整功能（需要自建后端）

除方案 A 外，还需配置：
- `base_url`
- `signin_path`
- `stats_path`
- `server_status_api_base_url`
- `server_status_api_path`
- `rcon_api_base_url`
- `rcon_password`
- `rcon_admin_ids`

---

## ⚙️ 配置说明（按模块）

插件配置入口：  
`AstrBot WebUI -> 插件 -> astrbot_plugin_hlymcn_signin`

### 🌐 1. 后端基础配置

| 配置项 | 说明 |
|--------|------|
| `base_url` | 后端基础地址（签到 / 数据查询默认基于此） |
| `timeout_ms` | HTTP 超时毫秒 |
| `signin_path` | 签到接口路径 |
| `stats_path` | 数据查询接口路径 |

### 👥 2. 群聊触发控制

| 配置项 | 说明 |
|--------|------|
| `allowed_group_ids` | 允许生效的群号列表 |
| `strict_single_plain` | 是否只允许纯文本消息触发 |
| `stats_keywords` | 数据查询触发词（支持多个） |

### 🖥️ 3. 查服配置

| 配置项 | 说明 |
|--------|------|
| `servers` | 服务器别名列表（`别名=ip:port`） |
| `players_limit` | 玩家列表显示上限 |
| `a2s_render_mode` | 查服输出模式：`card` / `image_text` / `text` |
| `server_status_api_base_url` | 查服 API 基础地址（留空默认尝试用 `base_url`） |
| `server_status_api_path` | 查服 API 路径 |
| `server_status_api_timeout_ms` | 查服 API 超时 |
| `server_status_api_fallback` | API 失败时是否回退 A2S 直连 |
| `a2s_timeout_ms` | A2S 超时 |
| `server_official_site` | 默认官网字段显示 |
| `server_official_sites` | 服务器专属官网（`别名=官网`） |

### 🖼️ 4. 查服背景模式

| 配置项 | 说明 |
|--------|------|
| `server_query_bg_mode` | `header` 或 `map_cover` |

模式行为：
- `header`：始终使用插件自定义头图缓存
- `map_cover`：优先使用后端返回地图封面；不可用时自动回退 `header`
- `map_cover`：插件会将已获取的地图封面缓存到 `data/plugin_data/<插件名>/map_cover_cache/`，文件名按地图名生成（例如 `de_dust2.jpg`）

> [!IMPORTANT]
> `map_cover` 需要后端查服 API 返回地图图字段之一：  
> `mapImage` / `map_image` / `basic_info.map_image`  
> 若后端不返回，插件会自动回退头图模式。

### 🗂️ 5. 头图缓存配置

| 配置项 | 说明 |
|--------|------|
| `header_image_url` | 头图链接（多条可用换行/逗号/分号分隔） |
| `header_prefetch_count` | `prefetch_header` 命令预载数量 |
| `header_cache_limit` | 头图缓存上限 |
| `header_image_max_bytes` | 单张头图下载最大字节 |

### 🔐 6. RCON 配置

| 配置项 | 说明 |
|--------|------|
| `rcon_api_base_url` | RCON API 基础地址 |
| `rcon_password` | RCON 密码 |
| `rcon_command_prefix` | RCON 指令前缀（默认 `rcon`） |
| `rcon_timeout_ms` | RCON 超时 |
| `rcon_admin_ids` | 允许执行 RCON 的管理员 QQ 列表（为空则禁用） |

### 🐞 7. 调试配置

| 配置项 | 说明 |
|--------|------|
| `debug_log` | 输出调试日志到控制台 |

---

## ⌨️ 命令与触发词

### ✍️ 签到
```text
签到
```

### 🔎 查服
```text
cs
```

### 📄 查看服务器别名列表
```text
ip
```

### 📊 数据查询
```text
data
stat
我的数据
查询数据
```

### 🛠️ RCON
```text
rcon <别名> <命令>
```
示例：
```text
rcon cs status
```

### 🧰 预载头图缓存
```text
prefetch_header
```

---

## 🔌 后端接口契约（最低要求）

### 1. 签到接口（`signin_path`）

- 请求：`GET`
- 关键参数：`userId`、`deviceType`、`signinType`
- 返回示例：
```json
{
  "message": "签到成功"
}
```

### 2. 数据查询接口（`stats_path/{qq}`）

- 请求：`GET`
- 返回示例（最少字段）：
```json
{
  "success": true,
  "data": {
    "steamId64": "7656xxxxxxxxxxxxxx",
    "credits": 123,
    "playTime": {
      "playerName": "Player"
    },
    "stats": {
      "kills": 0,
      "deaths": 0
    }
  }
}
```

### 3. 查服接口（`server_status_api_path`）

- 请求：`GET`
- 参数：`servers=ip:port`
- 返回示例（最少字段）：
```json
{
  "servers": [
    {
      "name": "Server Name",
      "map": "de_dust2",
      "mapImage": "https://example.com/cover.jpg",
      "raw": {
        "playerCount": 10,
        "maxPlayers": 32,
        "botCount": 0,
        "game": "Counter-Strike 2"
      },
      "query": {
        "ping": 35
      },
      "players": []
    }
  ]
}
```

### 4. RCON 接口（固定 `${rcon_api_base_url}/rcon`）

- 请求：`POST`
- 请求体：
```json
{
  "ip": "1.2.3.4:27015",
  "cmd": "status",
  "passwd": "your-password"
}
```

---

## 🧭 常见配置盲区

1. 配了 `map_cover` 但背景不生效  
后端没返回地图封面字段，插件会自动回退头图。

2. 配了 `servers` 但查服失败  
请检查服务器地址格式是否为 `ip:port`，并确认 `allowed_group_ids` 包含当前群。

3. 能查服但签到/查数据失败  
说明后端对应接口未实现或路径不匹配；查服 A2S 可独立工作，不代表其它接口可用。

4. RCON 没反应  
检查 `rcon_admin_ids` 是否包含你的 QQ，且 `rcon_api_base_url` 与 `rcon_password` 正确。

---

## 📄 License

GPL-3.0

---

> Made for AstrBot ❤️
